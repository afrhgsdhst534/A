...
public class InventoryDragger : MonoBehaviour
{
   ...
   public  float clickdelay = 0.1f;
    ...
    void Update()
    {
        ...
        {
            SDfDSgD = DSHG();
            if (SDfDSgD != null && SDfDSgD.GetComponent<Item>().ally)
            {
                DSgsdg = true;
        ...
        if (Input.GetButtonUp("Fire1"))
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, LayerMask.GetMask("Shop")) && SDfDSgD != null)
            {
                var l = SDfDSgD.GetComponent<Item>().player;
                l.GetComponent<InventoryManager>().Sell(SDfDSgD.GetComponent<Item>());
            }
            if (SDfDSgD != null && SDfDSgD.GetComponent<Item>().ally)
            {
                var dsF = DSHG();
                if (dsF != null)
                {
                    ...
                    var spells = canvasManager.pickedChar.GetComponent<InventoryManager>().items;
                    var spel = spells[f];
                    spells[f] = spells[s];
                    spells[s] = spel;
